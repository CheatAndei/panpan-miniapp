<template>
  <view class="page page-bottom-safe review-page">
    ...
    <view v-for="item in items" :key="item.submission.id" class="review-card">
      ...
      <button v-if="!item.photoExpanded" class="photo-loader" @tap="expandPhotos(item)">展开学生作答图片（{{ item.submission.attachments?.length || 0 }} 张）</button>
      <view v-else class="student-photo-panel">
        ...
        <view v-if="item.photoLoading" class="student-photo-state">正在读取学生图片…</view>
        ...
        <button v-else class="student-photo-retry" :disabled="item.photoLoading" @tap="loadPhotos(item)">图片未读取成功，点击重试</button>
      </view>
      <view v-if="item.photoExpanded && item.photosLoaded && !canReviewItem(item)" class="photo-error" role="alert">
...
async function loadPhotos(item) {
  if (!item || item.photoLoading || item.photosLoaded) return;
  item.photoLoading = true;
  try {
    const attachments = item.submission?.attachments || [];
    item.localPhotos = await Promise.all(attachments.map((photo) => api.downloadPrivate(photo.url).catch(() => '')))
      .then((list) => list.filter(Boolean));
    item.photosLoaded = attachments.length === 0 || item.localPhotos.length > 0;
  } finally { item.photoLoading = false; }
}
...
async function load() {
  ...
  try {
    ...
    const loadedItems = (data.submissions || []).map((item, index) => ({
      ...
      photosLoaded:false,
      photoLoading:false,
      photoExpanded:index===0,
      activePhotoIndex:0,
    }));
    if (loadedItems[0]) await loadPhotos(loadedItems[0]);
