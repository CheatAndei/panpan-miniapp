...
<view class="page">
  ...
  <view v-if="filtered.length===0" class="card"><view class="empty">暂无请假</view></view>
  ...
  methods:{
    async loadData(){
      try{
        ...
        const p=new Promise((r,j)=>{uni.request({url:BASE+'/leaves',header:{Authorization:`Bearer ${t}`},success(res){if(res.statusCode===200)r(res.data);else j(res.data);},fail:j});});
        const data=await p;
    ...
    async handle(id,status){
      try{
        const t=uni.getStorageSync('token');
        const p=new Promise((r,j)=>{uni.request({url:BASE+'/leaves/'+id,method:'PUT',data:{status,reply:status==='approved'?'收到，好好休息':'好的'},header:{Authorization:`Bearer ${t}`},success(res){if(res.statusCode===200)r(res.data);else j(res.data);},fail:j});});
        await p;
