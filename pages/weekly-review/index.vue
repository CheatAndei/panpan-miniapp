...
async function review(item,isCorrect){try{await api.put(`/weekly-challenge/v2/teacher/submissions/${item.submission.id}/review`,{is_correct:isCorrect,teacher_note:item.note});uni.showToast({title:'批阅已保存',icon:'success'});await load();}catch(e){uni.showToast({title:e?.error||'保存失败',icon:'none'});}}
