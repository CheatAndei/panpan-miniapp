<template>
  <view class="page page-bottom-safe">
    ...
    <view v-for="item in items" :key="item.submission.id" class="review-card">
      ...
      <view class="result-actions"><button class="wrong" @tap="review(item,false)">需要订正</button><button class="correct" @tap="review(item,true)">挑战成功</button></view>
    </view>
  </view>
</template>
...
async function review(item,isCorrect){try{await api.put(`/weekly-challenge/teacher/submissions/${item.submission.id}/review`,{is_correct:isCorrect,teacher_note:item.note});uni.showToast({title:'批阅已保存',icon:'success'});await load();}catch(e){uni.showToast({title:e?.error||'保存失败',icon:'none'});}}
</script>
