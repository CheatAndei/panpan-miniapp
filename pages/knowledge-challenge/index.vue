<template>
  <view class="page page-bottom-safe">
    ...
    <template v-if="selectedTopic&&!attempt">
      <view class="knowledge-card"><text class="knowledge-kicker">知识卡</text><text class="knowledge-title">{{ selectedTopic.title }}</text><text class="knowledge-copy">{{ selectedTopic.knowledge_card }}</text><button class="start-btn" :disabled="starting" @tap="start">{{ starting?'准备中…':'开始 8 题闯关' }}</button><button class="back-link" @tap="selectedTopic=null">返回知识点列表</button></view>
    </template>
    <template v-if="attempt&&!finished">
      ...
      <view class="question-card">
        ...
        <button v-for="(value,key) in currentQuestion.options" :key="key" :class="['option',{selected:selected===key,correct:answerResult&&key===answerResult.correct_option,wrong:answerResult&&key===selected&&!answerResult.is_correct}]" :disabled="submitting||Boolean(answerResult)" @tap="answer(key)"><text class="option-key">{{ key }}</text><text>{{ value }}</text></button>
      </view>
      <view v-if="answerResult" :class="['answer-card',{wrong:!answerResult.is_correct}]"><text class="answer-title">{{ answerResult.is_correct?'回答正确':'这题需要再巩固' }}</text><text class="answer-copy">{{ answerResult.explanation }}</text><button class="next-btn" @tap="next">{{ answerResult.completed?'查看本轮结果':'下一题' }}</button></view>
    </template>
