...
async function loadNext() {
  ...
  try {
    ...
    const result = await api.get(`/choice-king/next?student_id=${encodeURIComponent(id)}`);
...
function previewQuestion() {
  if (questionImage.value) uni.previewImage({ urls: [questionImage.value], current: questionImage.value });
}
...
function openLeaderboard() {
  ...
  uni.navigateTo({ url: `/pages/choice-king/leaderboard?student_id=${studentId.value}` });
}
</script>
