...
function templateData(pairs) {
  ...
  for (const [key, value] of pairs) {
    ...
    data[key] = { value: String(value || '').slice(0, max) };
  }
  return data;
}
...
function checkoutData(studentName, now, note = '') {
  ...
  if (isSpecial && FIELDS.checkout.note) data[FIELDS.checkout.note] = { value: String(note).slice(0, 20) };
  return data;
}
...
router.notifyReminder = async (classId, options = {}) => {
  ...
  const classTime = [details.classDate || bjDate(), details.startTime || ''].filter(Boolean).join(' ');
  return notifyParentsByClass(classId, TPLS.reminder, templateData([
    ...
    [FIELDS.reminder.note, details.note || '']
