...
async function notifyParents(studentId, tplId, data, page, templateName = '服务通知') {
  const db = getDB();
  const parents = db.all(`SELECT u.openid FROM users u JOIN bindings b ON b.parent_id=u.id
    JOIN students s ON s.id=b.student_id LEFT JOIN classes c ON c.id=s.class_id
    WHERE b.student_id=? AND s.deleted_at IS NULL AND c.deleted_at IS NULL
      AND u.openid IS NOT NULL AND u.openid!=''`, [studentId]);
...
router.notifyCheckin = async (studentId) => {
  const db = getDB();
  const s = db.get('SELECT name FROM students WHERE id=? AND deleted_at IS NULL', [studentId]);
  const now = wxTime();
...
router.notifyHomework = async (studentId, batchId, title = '作业批改已完成') => {
  const db = getDB();
  const student = db.get('SELECT name FROM students WHERE id=? AND deleted_at IS NULL', [studentId]);
...
router.notifyHomeworkParent = async (parentId, studentId, batchId, title = '作业批改已完成') => {
  const db = getDB();
  const student = db.get(`SELECT s.name FROM students s JOIN bindings b ON b.student_id=s.id
    LEFT JOIN classes c ON c.id=s.class_id
    WHERE s.id=? AND b.parent_id=? AND s.deleted_at IS NULL AND c.deleted_at IS NULL`, [studentId, parentId]);
  if (!student) return { ok: false, skipped: true, error: '学生已删除或绑定已失效' };
...
async function notifyParentsByClass(classId, tplId, data, page, templateName = '服务通知') {
  const db = getDB();
  const parents = db.all(`SELECT DISTINCT u.openid FROM users u JOIN bindings b ON b.parent_id=u.id
    ...
    WHERE s.class_id=? AND s.deleted_at IS NULL AND c.deleted_at IS NULL
      AND u.openid IS NOT NULL AND u.openid!=''`, [classId]);
  const result = { ok: true, total: parents.length, sent: 0, failed: 0, errors: [] };
  if (!tplId) return { ...result, ok: false, error: `${templateName}模板未配置` };
  if (parents.length === 0) {
