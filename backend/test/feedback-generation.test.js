...
test('学习小组反馈提示词用真实 emoji，不再发送 [加油] 死文本', async () => {
  ...
  assert.match(capturedRequest.messages[0].content, /120-160字/u);
  assert.match(capturedRequest.messages[0].content, /全文必须放得进课堂简报/u);
  assert.doesNotMatch(capturedRequest.messages[0].content, /200-300字/u);
});
