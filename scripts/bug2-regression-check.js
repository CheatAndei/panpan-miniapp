...
assert('通知时间字段兼容 time1-time20 且值为 HH:mm', notify.includes('for (let i = 1; i <= 20; i++) data[`time${i}`]') && notify.includes('function wxTime') && notify.includes('[fields.time, wxTime()]'));
assert('通知状态字段兼容 phrase1-phrase20', notify.includes('for (let i = 1; i <= 20; i++) data[`phrase${i}`]'));
assert('通知内容字段兼容 thing1-thing20', notify.includes('for (let i = 1; i <= 20; i++) data[`thing${i}`]'));
