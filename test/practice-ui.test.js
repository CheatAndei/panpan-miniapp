...
test('计划页使用四类可选初中计算题库，批改台独立且只处理未批改提交', () => {
  ...
  assert.ok(review.indexOf('photo-pane') < review.indexOf('answer-pane'), '照片必须位于答案左侧');
  ...
  assert.match(review, /activeSubmission\._saving \? '保存中…' : '保存'/);
  assert.match(review, /'预览'/);
  assert.match(review, /保存相册/);
  assert.match(review, /下一位/);
  assert.match(review, /rotateCurrentPhoto/);
  assert.match(review, /grid-template-columns:minmax\(0,1\.16fr\) minmax\(0,\.84fr\)/);
